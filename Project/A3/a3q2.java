 ...
 *
 * This algorithm similar to A1q1. I add new variable type named Variable,
 * it contained value and domain for the value.(I also add constraint as parameter for Variable, but DOESN'T use this parameter in this algorithm)
 *
 * This algorithm will give a list of action from the allowed domain in each variable to fill the first blank position
 * The result function will check if the action is not 0, then do the action to the first blank position
 * if the action is 0, means all blank position are filled but sill not touch the goal, then go back to the first blank position and try the domain again
 */
