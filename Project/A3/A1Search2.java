...
/**
 * This is the SearchResult class,
 * which is translated from A1 solution
 * Translate the Python to Java version
 */
class SearchResult{
...
class Search {
    ...
    SearchNode childNode(SearchNode parent_node, Actions action){
    ...
    SearchResult tree_search(State initial_state){
        ...
        while (!this.frontier.is_empty() && ((now - start_time) < this.timelimit) ){
            ...
            if (this.problem.is_goal(this_node.state)){
                ...
                for (Actions act:problem.actions(this_node.state)
                ) {
